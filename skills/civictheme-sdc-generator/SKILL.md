...
description: Generate all files for a new CivicTheme-compatible SDC component inside a Drupal sub-theme – NOT the CivicTheme UIKit. Sub-theme signal – the repo has no `packages/sdc/`; if `packages/sdc/` is present this is the UIKit, so use `civictheme-uikit-component-generator` instead. Use when the type-selector returns new_sdc_component, or when the user asks to create a new component, atom, molecule, or organism that doesn't exist in CivicTheme. Always use this skill before civictheme-paragraph-generator when a new component is also needed. Skip if the component is portable / self-contained – i.e. declares its own `--[prefix]-*` CSS token namespace, uses `var(--ct-*, #fallback)` with hardcoded fallbacks, and is explicitly designed for multi-site use. Portable components intentionally bypass the CivicTheme mixin system and are out of scope for every CivicTheme skill.
---
...
**Never declare `attributes` under `props`.** It is the reserved `Drupal\Core\Template\Attribute` object, injected automatically at render; declaring it (as a string or as the Attribute type) fails Drupal/CivicTheme SDC, and no CivicTheme component declares it. Render it on the root element with the guard instead – `{% if attributes is defined and attributes is not null %}{{- attributes -}}{% endif %}` (see `alert.twig`). `civictheme-health-check` fails any `*.component.yml` that declares it.
