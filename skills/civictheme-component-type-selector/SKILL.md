...
description: >
  Use this skill whenever the user mentions any change to a CivicTheme component or sub-theme — including bare phrases like "fix the [component]", "tweak the [component]", "update the [component]", "adjust the [component] styles", "change the [component] colours", "wire up the SCSS for X", "the [component] looks wrong", "the [component] isn't working", "make X author-configurable in CKEditor", or "Storybook of X is broken". Also triggers for the explicit phrasing already covered: "create a component", "override a component", "style a component", "scaffold a component", "add a paragraph type", "create a content element", "add sortable table", "add filterable table", "add JS enhancement", or any mention of CivicTheme SDC work. This is the required entry point for sub-theme work — always classify before generating, even when the user names a downstream skill directly. If the user says "use the civictheme-X skill" or pastes a slash command, still run the type-selector first to capture project context (theme machine name, paths, CivicTheme version) — downstream skills require these and will fail or fabricate values without them. Dispatch this skill via the Skill tool; do not substitute a Read of the SKILL.md file. Handles Drupal sub-theme work only — not UIKit source authoring. For new components in the CivicTheme UIKit, design system, design library, or component library itself, redirect to civictheme-uikit-component-generator. For SCSS modification of an existing UIKit component (spacing, colour, layout, selector-scoped overrides), redirect to civictheme-uikit-scss-iteration. For diagnostics or a sanity-check report, redirect to civictheme-health-check.
---
