...
### Step 5 – reserved-prop check (`attributes` declared under `props`)

`attributes` is the reserved `Drupal\Core\Template\Attribute` object – SDC injects it automatically at render. Declaring it under `props` in a `*.component.yml` fails Drupal/CivicTheme SDC. No CivicTheme component declares it. This step is the defence-in-depth tripwire for when an agent bypasses the generators and hand-authors a component (the failure this package exists to prevent).

Runs in both contexts whenever the repo contains `*.component.yml` files. Search roots:

- UIKit: `packages/sdc/components/` (source of truth – `packages/twig/` has no `.component.yml`).
- Sub-theme: the sub-theme `components/` tree.

Procedure:

1. Find every `*.component.yml`. If none exist, report `not_configured`.
2. For each, parse the YAML and inspect the `props` block. **Fail** if `attributes` appears as a declared property – either directly under `props:` or under `props.properties:` (the JSON-schema shape CivicTheme uses, e.g. `props: { type: object, properties: { attributes: { type: string } } }`). A bare grep for `attributes:` is too loose – confirm it sits inside the `props` schema, not in `slots`, `libraryOverrides`, or a prop *example*.
3. For each hit, record the file, the offending key path (`props.properties.attributes`), and the fix:
   - Remove `attributes` from `props`.
   - Render it on the root element with the guard: `{% if attributes is defined and attributes is not null %}{{- attributes -}}{% endif %}` (see `alert.twig`).
  ...
  - name: a11y_grep
    ...
    findings:
      ...
      - rule: "C"
        ...
        candidates: true
        hits: []
  - name: reserved_prop_check
    description: "attributes declared under props in a *.component.yml"
    status: fail
    failures:
