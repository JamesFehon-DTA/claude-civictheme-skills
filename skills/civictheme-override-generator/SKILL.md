...
- New props may be added after the verbatim copy of base props – but **never add `attributes` to `props`.** It is the reserved `Drupal\Core\Template\Attribute` object, injected automatically; declaring it fails SDC. Keep rendering it on the root with the guard `{% if attributes is defined and attributes is not null %}{{- attributes -}}{% endif %}` (the verbatim base copy already does this). `civictheme-health-check` fails any `*.component.yml` that declares it under `props`.
